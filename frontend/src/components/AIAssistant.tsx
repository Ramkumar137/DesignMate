...
import { Send, Loader2, Image, Copy } from "lucide-react";
...
export function AIAssistant({ uploadedImage, onPromptGenerated }: AIAssistantProps) {
  ...
  const handleAnalyzeImage = async () => {
    ...
    setIsLoading(true);
    const analyzeMsg = { role: "user", text: "Analyze this uploaded image and write a prompt or description for it" };
    ...
    try {
      ...
      formData.append('message', 'Analyze this uploaded image and write a detailed prompt or description for generating a UI/UX design based on this sketch. Focus on design elements, layout, colors, and user experience aspects.');

      const apiResponse = await fetch(API_ENDPOINTS.AI_ASSISTANT.replace('/chat', '/analyze-image'), {
        method: 'POST',
        body: formData,
      });
      ...
      if (result.status === 'ok' && result.data && result.data.response) {
        ...
        // Call the callback to set the generated prompt
        if (onPromptGenerated) {
          onPromptGenerated(result.data.response);
        }
        
        toast.success("Image analyzed! Prompt generated and copied to description field.");
      } else {
      ...
      const errorReply = {
        role: "assistant",
        text: "Sorry, I couldn't analyze the image. Please try again or describe your design manually.",
      };
